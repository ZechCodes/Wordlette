...
from wordlette.databases.drivers import AbstractDatabaseDriver
...
class DatabaseController(AutoInject):
    ...
    async def connect(
        ...
        driver = AbstractDatabaseDriver.__drivers__.get(db_settings.driver)
        if driver is None:
            raise RuntimeError(
        ...
        self._driver = driver()
        get_repository().set(AbstractDatabaseDriver, self._driver)
